...
public class UsuariosClientService(HttpClient client)
{
    ...
    {
        var response = await client.PostAsJsonAsync($"api/usuarios", usuario); return response.IsSuccessStatusCode;
    }

    public async Task<bool> PutAsync(Usuario usuario)
    {
        var response = await client.PutAsJsonAsync($"api/usuarios/{usuario.Email}", usuario); return response.IsSuccessStatusCode;
    }
    
    public async Task<bool> DeleteAsync(string email)
    {
        var response = await client.DeleteAsync($"api/usuarios/{email}"); return response.IsSuccessStatusCode;
    }
}